...
Launches a local web UI to visualize Likert-scale survey results per plantel.
Usage: python app.py
...
def api_planteles():
    return jsonify(planteles)


@app.route('/api/data/<plantel>')
def api_data(plantel):
    sub = df[df['plantel'] == plantel]
    total = len(sub)
    results = []
...
function renderStats() {
  ...
  row.innerHTML = '';
  // Compute average satisfaction across likert5 questions
...
function renderCharts() {
  ...
  if (!likert5.length && yesno.length) updateLegend('yesno');
}
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("  Evaluación de Calidad – Dashboard")
